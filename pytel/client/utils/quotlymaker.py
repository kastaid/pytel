...
async def quotlymaker(
    ...
    try:
        if isinstance(
            ...
            namewidth = (
                fallback.getsize(nm)[0]
                + 45
            )

        elif isinstance(
            user.sender_chat, Chat
        ):
            _CG = [
                ChatType.GROUP,
                ChatType.SUPERGROUP,
    ...
    # Profile Photo BG
    pfpbg = Image.new(
        "RGBA", (125, 600), (0, 0, 0, 0)
    )

    # Draw Template
    top, middle, bottom = await drawer(
        width, height
    )
    ...
    canvas.paste(
        middle,
        (pfpbg.width, top.height),
    )
    canvas.paste(
        bottom,
        (
            pfpbg.width,
            top.height + middle.height,
        ),
    )
    y = 90
...
async def no_photo(reply, nm):
    pfp = Image.new(
        "RGBA", (105, 105), (0, 0, 0, 0)
    )
    pen = ImageDraw.Draw(pfp)
    color = random.choice(COLORS)
    pen.ellipse(
        (0, 0, 105, 105), fill=color
    )
    letter = "" if not nm else nm[0]
    font = ImageFont.truetype(
        "resources/fonts/RobotoRegular.ttf",
        60,
    )
...
async def emoji_fetch(emoji):
    ...
    return await transparent(
        urllib.request.urlretrieve(
            img, "resources/emoji.png"
        )[0]
    )


async def transparent(emoji):
    emoji = Image.open(emoji).convert(
        "RGBA"
    )
